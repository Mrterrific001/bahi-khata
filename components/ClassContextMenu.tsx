...
import { Pin, Info, X, Trash2 } from 'lucide-react';
    ...
    <>
      ...
      <div className="fixed z-[70] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 bg-white rounded-2xl shadow-2xl p-2 animate-in zoom-in-95 duration-200">
         <div className="px-4 py-3 border-b border-slate-100 mb-1 flex justify-between items-center">
            <h3 className="font-bold text-slate-800 truncate">{classNameStr}</h3>
         ...
         <div className="space-y-1">
            ...
            >
               <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isPinned ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}>
                  <Pin className={`w-4 h-4 ${isPinned ? 'fill-current' : ''}`} />
               </div>
               <span className="font-medium text-slate-700">{isPinned ? 'Unpin Class' : 'Pin Class'}</span>
            </button>
            <button 
            ...
            >
               ...
               <span className="font-medium text-slate-700">Class Info & Stats</span>
            </button>
            ...
            >
               ...
               <span className="font-medium text-red-600">Delete Class</span>
            </button>
         </div>
      </div>
    </>
