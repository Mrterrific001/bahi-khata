...
import { Share, X, Trash2, Pin } from 'lucide-react';
...
export const BusinessContextMenu: React.FC<BusinessContextMenuProps> = ({ 
  isOpen, 
  onClose, 
  businessName,
  isPinned,
    ...
    <>
      ...
      <div className="fixed z-[70] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 bg-white rounded-2xl shadow-2xl p-2 animate-in zoom-in-95 duration-200">
         ...
         <div className="space-y-1">
            ...
            >
               <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isPinned ? 'bg-violet-100 text-violet-600' : 'bg-slate-100 text-slate-500'}`}>
                  <Pin className={`w-4 h-4 ${isPinned ? 'fill-current' : ''}`} />
               </div>
               <span className="font-medium text-slate-700">{isPinned ? 'Unpin Workspace' : 'Pin Workspace'}</span>
            </button>
            <button 
